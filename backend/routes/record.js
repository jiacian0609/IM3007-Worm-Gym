...
router.post('/', async (req, res) => {
	...
	if (sd !== undefined) {
		...
		for (let j = 0; j < record_list.length; j++) {
			...
			if (input_date.toISOString().substring(0, 10) === r.r_date) {
				input_day = r.day;
			}
		...
		let p_item = await pool.query(`SELECT equip_id, reps, sets
									   FROM "WormGym".fitness_program
									   WHERE user_id = $1 AND "date" = $2 AND "Day" = $3 AND equip_id = $4`, [user_id, sd, input_day, equip_id]);
		p_item = p_item.rows
		if (p_item.length > 0) {
			p_item = p_item[0];
			p_item.reps = p_item.reps.split(" ")[0];  // remove unit
			if (reps >= p_item.reps && sets >= p_item.sets) {
